...
**Notebooks**
- `notebooks/experimental/04_embeddings-demo.ipynb` - Demo con análisis cuantitativo
...
| **B08** | **NIR Broad** | **10m** | **842 nm** | **NDVI** |
| **B8A** | **NIR Narrow** | **20m** | **865 nm** | **Prithvi** |
| B11 | SWIR1 | 20m | 1610 nm | Prithvi |
| B12 | SWIR2 | 20m | 2190 nm | Prithvi |

**IMPORTANTE:**
- **B08 (NIR Broad, 10m)**: Banda estándar para cálculo de NDVI
- **B8A (NIR Narrow, 20m)**: Banda requerida por modelo Prithvi
- Ambas bandas NIR se descargan para completar el flujo de análisis
...
notebooks/experimental/04_embeddings-demo.ipynb
...
- Add 04_embeddings-demo.ipynb with quantitative analysis (AGENTS.md compliant)
...
## Addendum: Inclusión de Banda B08 (Enero 2025)

**Fecha**: 2025-01-11
**Razón**: Durante US-008 se identificó que faltaba B08 para cálculo correcto de NDVI

### Problema Identificado

El sistema original descargaba **B8A (NIR Narrow, 20m)** para Prithvi pero **NO** descargaba **B08 (NIR Broad, 10m)**, que es la banda estándar para NDVI. Esto causaba que el NDVI se calculara interpolando B8A en lugar de usar B08 nativa.

### Solución Implementada

1. **Modificación de `download_hls_bands()`**:
   - Agregada B08 a las bandas de 10m
   - `bands_10m`: `['B02', 'B03', 'B04', 'B08']` (antes solo 3 bandas)
   - `bands_20m`: `['B8A', 'B11', 'B12']` (sin cambios)

2. **Script de Descarga Complementario**:
   - `scripts/download_b08.py`: Descarga solo B08 para zonas existentes
   - No requiere re-descargar todas las bandas

3. **Notebook Actualizado**:
   - Lógica de fallback: Usa B08 si existe, sino interpola B8A
   - Compatible con datos antiguos y nuevos

### Diferencia B08 vs B8A

| Banda | Resolución | Wavelength | Uso Principal |
|-------|-----------|------------|---------------|
| **B08** | 10m | 842 nm | **NDVI estándar** |
| **B8A** | 20m | 865 nm | **Prithvi embeddings** |

**Justificación**: B08 es el estándar de la industria para NDVI por su resolución 10m y rango espectral más amplio.

### Impacto

- ✅ **Futuras descargas**: Incluyen B08 automáticamente
- ⚠️ **Datos existentes**: Ejecutar `scripts/download_b08.py` para complementar
- ✅ **Compatibilidad**: Código funciona con/sin B08 (fallback automático)
...
- **Actualización 2025-01**: Incluye B08 para NDVI correcto
