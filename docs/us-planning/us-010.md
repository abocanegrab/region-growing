...
### Problema Actual
- **US-007 (MGRG) produce regiones sin etiquetas semánticas**: "Región 45" no es interpretable
- **Análisis de estrés se aplica a todas las regiones**: No distingue entre cultivos y zonas urbanas
- **Mapas no son autoexplicativos**: Difícil comunicar resultados a stakeholders no técnicos
- **Falta jerarquía**: Necesitamos clasificar PRIMERO la clase, LUEGO el estrés (solo para cultivos)

### Estado del Arte (2024-2025)
Según investigación de papers recientes:
- **Zero-Shot Classification**: SAM-CLIP 2024 logra +6.8% mIoU vs métodos anteriores
- **Prithvi-EO-2.0**: Mejor modelo en segmentación (Diciembre 2024), embeddings 256D ideales para clasificación
- **Hierarchical Classification**: Transfer learning con niveles (Coarse → Fine) mejora resultados
- **Agricultural Applications**: 70-75% accuracy en clasificación de cobertura terrestre

### Solución Propuesta
**Clasificación Jerárquica Zero-Shot con NDVI + Embeddings:**

**Nivel 1 - Coarse Classes (6 categorías):**
1. **Water** (Agua)
2. **Urban** (Área urbana/construida)
...
6. **Grass/Shrub** (Vegetación natural)

**Nivel 2 - Stress Analysis (solo para cultivos):**
- Bajo estrés: NDVI 0.5-0.6
- Medio estrés: NDVI 0.4-0.5
- Alto estrés: NDVI 0.3-0.4

**Ventajas:**
- **Simple**: Solo NDVI + embeddings (ya disponibles)
- **Rápido**: Heurísticas eficientes sin training
- **Interpretable**: Clases claras y útiles
...
- **Tiempo de clasificación**: <2s para 150 regiones
...
- [ ] Tests unitarios: >70% cobertura
  - Test clasificación por NDVI ranges
  - Test con embeddings sintéticos
  - Test edge cases (región vacía, single pixel)
  - Test jerarquía (cultivo → estrés)

### Cumplimiento AGENTS.md
- [ ] Código en inglés (funciones, variables)
- [ ] Documentación en español (narrativa)
...
1. INPUTS (De US Anteriores)
   │
   ├── Segmentación MGRG: (H, W) con IDs de región [0, N]
   ├── Embeddings Prithvi: (H, W, 256) features semánticos
   ├── NDVI: (H, W) valores [-1, 1]
   └── RGB (opcional): Para visualización

2. EXTRACCIÓN DE FEATURES POR REGIÓN
   │
   └── Para cada región R:
       ├── Crear máscara: mask = (segmentation == region_id)
       ├── Extraer embeddings: region_embs = embeddings[mask]
       ├── Calcular embedding promedio: mean_emb = region_embs.mean(axis=0)
       ├── Extraer NDVI: region_ndvi = ndvi[mask]
       ├── Calcular NDVI promedio: mean_ndvi = region_ndvi.mean()
       └── Calcular std NDVI: std_ndvi = region_ndvi.std()

3. CLASIFICACIÓN JERÁRQUICA
   │
   ├── NIVEL 1: Clase Coarse (basada en NDVI + heurísticas)
   │   │
   │   ├── IF mean_ndvi < 0.1:
   │   │   ├── Check embedding similarity to "urban" centroid
   │   │   ├── IF similar → Class = Urban (1)
   │   │   └── ELSE → Class = Water (0)
   │   │
   │   ├── ELIF 0.1 ≤ mean_ndvi < 0.3:
   │   │   └── Class = Bare Soil (2)
   │   │
   │   ├── ELIF 0.3 ≤ mean_ndvi < 0.6:
   │   │   └── Class = Stressed Crop (4)
   │   │
   │   └── ELIF mean_ndvi ≥ 0.6:
   │       └── Class = Vigorous Crop (3)
   │
   └── NIVEL 2: Stress Refinement (solo para cultivos)
       │
       └── IF Class in [Vigorous Crop, Stressed Crop]:
           ├── Bajo Estrés: 0.5 ≤ mean_ndvi < 0.6
           ├── Medio Estrés: 0.4 ≤ mean_ndvi < 0.5
           └── Alto Estrés: 0.3 ≤ mean_ndvi < 0.4
...
5. CROSS-VALIDATION (Opcional pero Recomendado)
   │
   └── IF Dynamic World available:
       ├── Load DW mask (aligned)
       ├── Map our classes to DW classes:
       │   ├── Water (0) → DW Water (0)
       │   ├── Urban (1) → DW Built Area (6)
       │   ├── Bare Soil (2) → DW Bare Ground (7)
       │   ├── Crops (3, 4) → DW Crops (4)
       │   └── Grass/Shrub (5) → DW Trees/Grass (1, 2)
       ├── Calculate agreement per class
       └── Report overall agreement (target: >70%)

6. VISUALIZACIÓN Y REPORTE
   │
   ├── Mapa Semántico Coloreado:
   │   ├── Water → Blue (#0077BE)
   │   ├── Urban → Gray (#808080)
   │   ├── Bare Soil → Brown (#8B4513)
   │   ├── Vigorous Crop → Dark Green (#228B22)
   │   ├── Stressed Crop → Yellow-Green (#9ACD32)
   │   └── Grass/Shrub → Light Green (#90EE90)
   │
   ├── Estadísticas por Clase:
   │   └── Tabla: Class | Objects | Area (ha) | Mean NDVI | Std NDVI
   │
   ├── Distribución Espacial:
   │   └── Histogram de NDVI por clase
   │
   └── Figura para Paper:
       ├── Panel 1: RGB original
       ├── Panel 2: Segmentación MGRG (regiones anónimas)
       ├── Panel 3: Mapa Semántico (clasificado)
       └── Panel 4: Dynamic World (referencia)
```
...
### Módulo: `src/classification/zero_shot_classifier.py`
...
- Segmentación MGRG (156 regiones)
...
- Histograma de tamaños de regiones
...
### 3.3 Resultados por Región (Ejemplo)
Region 5:
  Class: Vigorous Crop
  Confidence: 0.87
  Mean NDVI: 0.72
...
### 4.3 Visualización Comparativa
```
┌──────────────┬──────────────┬──────────────┐
│ RGB Original │ MGRG Seg     │ Semantic Map │
│              │ (anónimo)    │ (clasificado)│
└──────────────┴──────────────┴──────────────┘
```
...
### 5.2 Gráficos
- Barplot: Área por clase (hectáreas)
- Pie chart: Proporción de clases
- Boxplot: Distribución de NDVI por clase
...
Mean Agreement      : 73.4% ✅ (Target: >70%)

### 6.4 Discusión
- Alta concordancia en Water (92%) - clase bien definida
- Urban moderada (78%) - confusión con Bare Soil
- Crops acceptable (70-74%) - Dynamic World no distingue estrés
- Grass/Shrub baja (59%) - límites ambiguos con crops

## 7. Análisis por Zona

### 7.1 Repetir para Bajío
- Similar análisis
...
### 7.3 Comparación Agregada
Tabla: Zona | Vigorous Crop (ha) | Stressed Crop (ha) | Water (ha) | ...

## 8. Análisis Jerárquico (Clase → Estrés)

### 8.1 Filtrar Solo Cultivos
crop_regions = [r for r in classifications.values()
                if r.class_id in [3, 4]]

### 8.2 Sub-Clasificación de Estrés
Bajo Estrés  : 25 regions (450 ha)
Medio Estrés : 32 regions (680 ha)
Alto Estrés  : 48 regions (638 ha)

### 8.3 Mapa de Calor de Estrés
- Solo cultivos
- Gradiente verde → amarillo → rojo

## 9. Visualización para Paper

### 9.1 Figura Multi-Panel (300 DPI)
```
┌──────────────┬──────────────┬──────────────┐
│ RGB Original │ Segmentation │ Dynamic      │
│              │ (MGRG)       │ World (GT)   │
├──────────────┼──────────────┼──────────────┤
│ Semantic     │ Stress Map   │ Legend       │
│ Map (Ours)   │ (Crops only) │              │
└──────────────┴──────────────┴──────────────┘
```

### 9.2 Tabla de Clasificación (LaTeX)
...
1. Clasificación zero-shot logra 73% agreement con Dynamic World
2. Jerarquía Clase → Estrés corrige problema de US-008
3. Mapas semánticos más interpretables que regiones anónimas
...
### Fase 1: Implementación del Clasificador (3 horas)
...
- [ ] Implementar `__init__()` con validación de shapes
- [ ] Implementar `classify_region()` con heurísticas NDVI
- [ ] Implementar `_classify_coarse()` con lógica de decisión
- [ ] Implementar `classify_all_regions()` con loop y filtrado
- [ ] Implementar `generate_semantic_map()` para mapeo de clases
- [ ] Implementar `generate_colored_map()` para visualización RGB
- [ ] Implementar `get_class_statistics()` para análisis agregado
- [ ] Logging profesional (logger, no print)

**Criterios de éxito**:
- Código en inglés, docstrings estilo Google
- Type hints en todas las funciones
- Manejo de edge cases (regiones vacías, single pixel)
- Sin emojis en código

#### 1.2 Función de Cross-Validation (1h)
**Archivos**: `src/classification/zero_shot_classifier.py`

**Tareas**:
- [ ] Implementar `cross_validate_with_dynamic_world()`
- [ ] Definir mapeo de clases: Our → Dynamic World
- [ ] Calcular agreement por clase
- [ ] Calcular agreement global
- [ ] Retornar diccionario con resultados

**Criterios de éxito**:
- Mapeo correcto de clases (Water→0, Urban→6, Crops→4, etc.)
- Manejo de clases sin datos
- Documentación clara del mapeo

#### 1.3 Optimización y Refinamiento (30 min)
**Tareas**:
- [ ] Revisar complejidad temporal (debe ser O(n) para classify_all_regions)
- [ ] Optimizar cálculos con NumPy vectorizado
...
**Estructura de tests** (mínimo 20 tests):
...
- Uso de fixtures para datos sintéticos

#### 2.2 Tests de Integración (30 min)
...
**Secciones** (siguiendo estándar US-007/008):
...
2. **Carga de Datos** (2 celdas)
   - Cargar segmentación MGRG (156 regiones)
   - Cargar embeddings Prithvi (256D)
   - Cargar NDVI y RGB

3. **Clasificación Zero-Shot** (3 celdas)
   - Inicializar SemanticClassifier
   - Clasificar todas las regiones
   - Mostrar resultados de ejemplo (5 regiones)

4. **Generación de Mapa Semántico** (2 celdas)
   - Crear mapa de clases
   - Crear mapa coloreado RGB
   - Visualización comparativa (RGB | MGRG | Semantic)

5. **Estadísticas por Clase** (3 celdas)
...
7. **Análisis Jerárquico** (2 celdas)
   - Filtrar solo cultivos
   - Sub-clasificación de estrés (bajo/medio/alto)
   - Mapa de calor de estrés
...
#### 3.2 Aplicación a 3 Zonas (1h)
**Tareas**:
- [ ] Repetir análisis para Mexicali
- [ ] Repetir análisis para Bajío
- [ ] Repetir análisis para Sinaloa
...
**Secciones a agregar**:
- [ ] Título: "US-010: Clasificación Semántica de Objetos"
- [ ] Descripción del problema y solución
- [ ] Taxonomía de 6 clases LULC
...
**Criterios de éxito**:
- Sección completa (80-100 líneas)
- Ejemplos ejecutables
- Formato consistente con US-007/008

#### 4.2 Resolución de US (30 min)
**Archivos**: `docs/us-resolved/us-010.md`

**Estructura** (siguiendo template US-007/008):
- [ ] Resumen ejecutivo
- [ ] Criterios de aceptación cumplidos
- [ ] Implementación realizada (archivos creados/modificados)
- [ ] Resultados experimentales (tabla de métricas)
...
## 🧪 Especificación de Testing (>70% cobertura)
...
tests/
├── unit/
│   └── test_zero_shot_classifier.py    # 20+ tests unitarios
└── integration/
    └── test_classification_workflow.py  # 3+ tests de integración
```
...
- [ ] Todos los tests pasan (20+ unitarios, 3+ integración)
- [ ] Tiempo de ejecución <10s para tests unitarios
- [ ] Tiempo de ejecución <30s para tests de integración
...
## 📈 Métricas Esperadas y Benchmarks

### Resultados Esperados por Zona

**Basado en análisis preliminar de datos:**

| Zona | Regiones MGRG | Clases Esperadas | Agreement DW | Tiempo Clasificación |
|------|---------------|------------------|--------------|---------------------|
| **Mexicali** | 156 | Water (5%), Urban (8%), Bare Soil (15%), Crops (65%), Grass (7%) | 72-75% | <2s |
| **Bajío** | 120 | Water (2%), Urban (12%), Bare Soil (10%), Crops (70%), Grass (6%) | 70-73% | <2s |
...
|-------|-------------------|-------------------|-------|
| **Water** | 90-95% | 0.85-0.95 | Clase más fácil (NDVI muy bajo) |
| **Urban** | 75-80% | 0.70-0.80 | Confusión con Bare Soil |
| **Bare Soil** | 65-70% | 0.60-0.75 | Límites ambiguos con Stressed Crop |
| **Vigorous Crop** | 75-80% | 0.80-0.90 | Alta confianza (NDVI >0.6) |
| **Stressed Crop** | 68-73% | 0.65-0.75 | Overlap con Bare Soil y Grass |
| **Grass/Shrub** | 60-65% | 0.60-0.70 | Clase más difícil (heterogénea) |
...
|-----------|----------------|-----------------|
| **water_urban: 0.05** | Muy restrictivo, menos Water | Zonas áridas |
| **water_urban: 0.10** | Estándar (recomendado) | Uso general |
| **water_urban: 0.15** | Permisivo, más Water | Zonas húmedas |
| **bare_soil: 0.25** | Menos Bare Soil, más Stressed Crop | Agricultura intensiva |
| **bare_soil: 0.30** | Estándar (recomendado) | Uso general |
| **bare_soil: 0.35** | Más Bare Soil, menos Stressed Crop | Zonas secas |
| **stressed_crop: 0.55** | Menos Stressed, más Vigorous | Cultivos saludables |
| **stressed_crop: 0.60** | Estándar (recomendado) | Uso general |
| **stressed_crop: 0.65** | Más Stressed, menos Vigorous | Detección temprana estrés |

**Gráfico a generar en notebook:**
```
...
| Método | Agreement | Año | Referencia |
|--------|-----------|-----|------------|
| **Dynamic World (Google)** | 86% | 2022 | Brown et al. |
| **SAM-CLIP** | 78% | 2024 | Wang et al. |
| **Prithvi-EO-2.0 (fine-tuned)** | 82% | 2024 | Muhtar et al. |
| **Nuestro método (zero-shot)** | **70-75%** (target) | 2025 | Este proyecto |

**Interpretación:**
- Nuestro método es zero-shot (sin entrenamiento)
- 70-75% es competitivo para zero-shot
- Fine-tuning podría alcanzar 80%+ (trabajo futuro)

## ⚠️ Riesgos y Mitigaciones

### Riesgo 1: Baja precisión de clasificación (<65%)
**Probabilidad**: Media (30%)
**Impacto**: Alto (bloquea US)

**Mitigaciones**:
1. **Simplificar taxonomía**: Reducir a 4 clases (Water, Urban, Crop, Other)
2. **Clasificación binaria**: Crop vs Non-Crop como fallback
3. **Ajuste de thresholds**: Calibración por zona
4. **Uso de embeddings**: Agregar similitud coseno como feature adicional

**Plan B**: Si agreement <65%, usar clasificación jerárquica:
- Nivel 1: Vegetation vs Non-Vegetation (NDVI >0.3)
- Nivel 2: Sub-clasificación dentro de cada grupo

### Riesgo 2: Confusión Urban ↔ Bare Soil
**Probabilidad**: Alta (60%)
**Impacto**: Medio (afecta métricas pero no bloquea)

**Mitigaciones**:
1. **Usar std de embeddings**: Urban tiene mayor variabilidad espacial
2. **Análisis de textura**: Calcular entropía local
3. **Contexto espacial**: Urban suele estar en clusters compactos
4. **Threshold adaptativo**: Ajustar por zona (urbano vs rural)

**Validación**: Inspección visual de casos ambiguos en notebook

### Riesgo 3: Threshold NDVI muy sensible
**Probabilidad**: Media (40%)
**Impacto**: Medio (requiere calibración manual)

**Mitigaciones**:
1. **Análisis de sensibilidad**: Evaluar thresholds 0.05-0.35 en notebook
2. **Thresholds adaptativos**: Calcular percentiles por zona
3. **Intervalos de confianza**: Usar rangos en lugar de valores fijos
4. **Validación cruzada**: Comparar con Dynamic World para calibrar

**Implementación**:
```python
# Threshold adaptativo basado en distribución local
...
**Mitigaciones**:
1. **Validación visual**: Comparación manual con RGB
2. **Métricas internas**: Coherencia, consistencia temporal
3. **Uso de otras fuentes**: ESA WorldCover, ESRI Land Cover
4. **Validación cualitativa**: Análisis de casos de uso

### Riesgo 5: Tiempo de clasificación >5s
**Probabilidad**: Baja (15%)
**Impacto**: Bajo (no crítico para demo)

**Mitigaciones**:
1. **Vectorización NumPy**: Evitar loops Python
2. **Caching de estadísticas**: Pre-calcular mean/std por región
3. **Paralelización**: Usar multiprocessing para múltiples zonas
4. **Optimización de código**: Profiling con cProfile

---

## 🔮 Hallazgos Esperados y Lecciones Anticipadas

### Hallazgos Técnicos Esperados

#### 1. Separabilidad de Clases por NDVI
**Hipótesis**: Las clases se separan bien en el espacio NDVI

**Validación esperada**:
- Water: NDVI <0.1 (separación clara)
- Urban/Bare Soil: NDVI 0.1-0.3 (overlap esperado)
- Crops: NDVI >0.3 (separación clara)

**Métrica**: Calcular distancia entre distribuciones (Kolmogorov-Smirnov test)

#### 2. Rol de Embeddings Semánticos
**Hipótesis**: Embeddings ayudan a distinguir Urban vs Bare Soil

**Validación esperada**:
- Calcular similitud coseno intra-clase vs inter-clase
- Urban debería tener mayor variabilidad (std de embeddings)
- Bare Soil debería ser más homogéneo

**Métrica**: Ratio de varianza intra-clase / inter-clase

#### 3. Consistencia Espacial
**Hipótesis**: Clasificación mantiene coherencia espacial de MGRG

**Validación esperada**:
- Regiones MGRG no se fragmentan en múltiples clases
- Cada región tiene clase dominante >80%
- Transiciones suaves entre regiones vecinas

**Métrica**: Pureza de región (% de píxeles de clase dominante)

#### 4. Sensibilidad a Thresholds
**Hipótesis**: Thresholds NDVI son robustos ±0.05

**Validación esperada**:
- Variación de agreement <5% con threshold ±0.05
- Clases extremas (Water, Vigorous Crop) más robustas
- Clases intermedias (Bare Soil, Stressed Crop) más sensibles

**Métrica**: Curva de sensibilidad (agreement vs threshold)

### Lecciones Aprendidas Anticipadas

#### Lección 1: Zero-Shot es Viable pero Limitado
**Expectativa**: 70-75% agreement es competitivo para zero-shot, pero lejos de métodos supervisados (85%+)

**Implicación**: 
- Zero-shot es útil para prototipado rápido
- Fine-tuning necesario para producción
- Trade-off entre simplicidad y precisión

#### Lección 2: NDVI es Feature Dominante
**Expectativa**: NDVI explica >80% de la varianza en clasificación

**Implicación**:
- Embeddings semánticos son complementarios, no primarios
- Clasificación podría simplificarse a solo NDVI + heurísticas
- Valor de embeddings está en casos ambiguos (Urban vs Bare Soil)

#### Lección 3: Jerarquía Clase → Estrés es Correcta
**Expectativa**: Análisis jerárquico mejora interpretabilidad

**Implicación**:
- Primero identificar objeto (campo), luego analizar estrés interno
- Evita confundir "campo estresado" con "suelo desnudo"
- Alineado con workflow de agronomía real

#### Lección 4: Validación con Dynamic World es Imperfecta
**Expectativa**: Dynamic World tiene errores propios (10-15%)

**Implicación**:
- Agreement 70-75% puede ser subestimado
- Validación visual es necesaria
- Múltiples fuentes de ground truth son ideales

### Trabajo Futuro Identificado

#### Corto Plazo (US-011)
1. **Integración en Pipeline End-to-End**
...
7. **Transferencia a Otras Regiones**
   - Evaluar en Europa, África, Asia
   - Calibración automática de thresholds
   - Adaptación a diferentes climas

8. **Integración con Modelos de Cultivo**
   - Combinar clasificación con modelos agronómicos (DSSAT, APSIM)
   - Predicción de rendimiento por clase
   - Recomendaciones de manejo

## 📊 Métricas de Éxito de la US
...
- [ ] Documentación completa (README + resolución)
...
### Código
- [ ] `src/classification/zero_shot_classifier.py` implementado (700+ líneas)
- [ ] Clase `SemanticClassifier` completa con 9 métodos
- [ ] Función `cross_validate_with_dynamic_world()` implementada
- [ ] Type hints en todas las funciones
...
- [ ] Tiempo de ejecución <10s (unitarios), <30s (integración)
- [ ] Sin warnings de pytest
- [ ] Sin errores de tipo (mypy)
...
- [ ] Aplicación a 3 zonas (Mexicali, Bajío, Sinaloa)
- [ ] Visualizaciones profesionales (300 DPI)
- [ ] Estadísticas por clase (tabla + gráficos)
- [ ] Cross-validation con Dynamic World
- [ ] Análisis de sensibilidad de thresholds
- [ ] Conclusiones y recomendaciones
- [ ] Texto en español, código en inglés

### Documentación
- [ ] README actualizado con sección US-010 (80-100 líneas)
- [ ] Ejemplos de uso con código ejecutable
- [ ] Tabla de métricas típicas
- [ ] Referencias académicas (3+ papers)
...
### Pre-Desarrollo
- [ ] Leer y entender US-007 y US-008 (estándar de excelencia)
...
### Documentación
- [ ] Actualizar README con sección US-010
...
**Recursos**:
- Planeación: `docs/us-planning/us-010.md` (este archivo)
- Estándar: `AGENTS.md`
- Referencias: US-007, US-008 (ejemplos de excelencia)
- Papers: Ver sección "Referencias Académicas"

**Preguntas Frecuentes**:
1. **¿Qué hacer si agreement <70%?** → Ver sección "Riesgos y Mitigaciones"
2. **¿Cómo calibrar thresholds?** → Análisis de sensibilidad en notebook
3. **¿Qué hacer si Dynamic World no está disponible?** → Validación visual + métricas internas
4. **¿Cómo manejar casos ambiguos?** → Documentar en notebook, usar confianza baja
...
**Versión**: 2.0 (Mejorada con estándar US-007/008)
